...
if ($USER->IsAuthorized() || $arParams["ALLOW_AUTO_REGISTER"] == "Y" )
{
	if(strlen($_REQUEST["ORDER_ID"]) <= 0)
	{
		...
		while ($arItem = $dbBasketItems->GetNext())
		{
			if ($arItem["DELAY"] == "N" && $arItem["CAN_BUY"] == "Y")
			{
				...
				if($arItem["VAT_RATE"] > 0 && !CSaleBasketHelper::isSetItem($arItem))
				{
					...
					$v = roundEx((($arItem["PRICE"] * $arItem["QUANTITY"] / ($arItem["VAT_RATE"] + 1)) * $arItem["VAT_RATE"]), SALE_VALUE_PRECISION);
		...
		foreach ($arResult["BASKET_ITEMS"] as &$arResultItem)
		{
			...
			if (isset($arProductData[$arResultItem["PRODUCT_ID"]]))
			{
		...
		if ((int)$arUserResult["DELIVERY_LOCATION"] > 0)
		{
			...
			foreach($arDeliveryServiceAll as $deliveryObj)
			{
				if (count($arP2D[$arUserResult["PAY_SYSTEM_ID"]]) <= 0 || in_array($deliveryObj->getCode(), $arP2D[$arUserResult["PAY_SYSTEM_ID"]]))
				{
					...
					if($mustBeCalculated)
					{
						...
						{
							$arDelivery["PRICE_FORMATED"] = SaleFormatCurrency($calcResult->getPrice(), $arResult["BASE_LANG_CURRENCY"]);
							$arDelivery["CURRENCY"] = $arResult["BASE_LANG_CURRENCY"];
			...
			if(!$bSelected && !empty($arResult["DELIVERY"]))
			{
				...
				{
					$arUserResult["DELIVERY_ID"] = $arResult["DELIVERY"][$key]["ID"];
				}
				...
				if(!isset($arResult["DELIVERY"][$key]["PRICE"]))
				{
					...
					if($calcResult->isSuccess())
					{
						$arResult["DELIVERY"][$key]["PRICE_FORMATED"] = SaleFormatCurrency($calcResult->getPrice(), $arResult["BASE_LANG_CURRENCY"]);
						$arResult["DELIVERY"][$key]["CURRENCY"] = $arResult["BASE_LANG_CURRENCY"];
					...
					{
						foreach($calcResult->getErrorMessages() as $message)
							$arResult["DELIVERY"][$key]["CALCULATE_ERRORS"] .= $message.'<br>';
					}
					else
					{
						$arResult["DELIVERY"][$key]["CALCULATE_ERRORS"] = GetMessage("SOA_DELIVERY_CALCULATE_ERROR");
					}
				}
			}
		...
		{
			foreach($arResult["DELIVERY"] as $deliveryKey => $deliveryData)
			{
				if (!empty($deliveryData['PRICE']))
				{
		...
		{
			if (CSaleBasketHelper::isSetItem($arItem) && $isOrderConverted != 'Y')
		...
		else
		{
			...
			if($arUserResult["PAY_CURRENT_ACCOUNT"] == "Y")
			{
				...
				{
					$needSum = (($arResult["USER_ACCOUNT"]["CURRENT_BUDGET"] >= $orderTotalSum) ? $orderTotalSum : $arResult["USER_ACCOUNT"]["CURRENT_BUDGET"]);
					$arResult["PAYED_FROM_ACCOUNT_FORMATED"] = SaleFormatCurrency($needSum, $arResult["BASE_LANG_CURRENCY"]);

					$arResult["ORDER_TOTAL_LEFT_TO_PAY_FORMATED"] = SaleFormatCurrency(($orderTotalSum - $needSum), $arResult["BASE_LANG_CURRENCY"]);
				}
			}
		...
		if($arUserResult["CONFIRM_ORDER"] == "Y" && empty($arResult["ERROR"]))
		{
			...
			if($USER->IsAuthorized() && empty($arResult["ERROR"]))
			{
				...
				$isPayFromUserBudget = ($arResult["PAY_FROM_ACCOUNT"] == "Y" && $arUserResult["PAY_CURRENT_ACCOUNT"] == "Y"
        ...
        //+
        if (is_array($arParams["PROP_".$arFields["PERSON_TYPE_ID"]]) &&
             !empty($arParams["PROP_".$arFields["PERSON_TYPE_ID"]]))
        {
          $objProfile = CSaleOrderUserProps::GetList(array(),
          ...
          if ($objProfile->SelectedRowsCount() == 1 &&
              is_array($objProfile = $objProfile->Fetch()))
          {
              $arUserResult["PROFILE_ID"] = $objProfile["ID"];
              $arUserResult["PROFILE_NAME"] = $objProfile["NAME"];
            
              $dbPropVals = CSaleOrderUserPropsValue::GetList(
                              array(),
                              array("USER_PROPS_ID" => $objProfile["ID"]),
                              false,
                              false,
                              array("ID", "ORDER_PROPS_ID", "VALUE")
                            );
            
             while($arProp = $dbPropVals->Fetch())
             {
               if (in_array($arProp["ORDER_PROPS_ID"], $arParams["PROP_".$arFields["PERSON_TYPE_ID"]]) &&
                   !empty($arProp["VALUE"]) &&
                   !is_array($arProp["VALUE"]) &&
                   array_key_exists($arProp["ORDER_PROPS_ID"], $arOrderDat["ORDER_PROP"]))
               {
                 $arOrderDat["ORDER_PROP"][$arProp["ORDER_PROPS_ID"]] = $arProp["VALUE"];
                 $arUserResult["ORDER_PROP"][$arProp["ORDER_PROPS_ID"]] = $arProp["VALUE"];
               }
             }
          }
        }
          ...
          $OrderProps = array();
                ...
                !empty($arOrder))
            {
              if (intval($arOrder["DELIVERY_ID"]) > 0 &&
                  is_array($arResult["DELIVERY"][$arOrder["DELIVERY_ID"]]))
              {
                $DeliverySystem = $arResult["DELIVERY"][$arOrder["DELIVERY_ID"]]["NAME"];
              }
              
              if (intval($arOrder["PAY_SYSTEM_ID"]) > 0 &&
                  is_array($arResult["PAY_SYSTEM"][$arOrder["PAY_SYSTEM_ID"]]))
              {
                $PaySystem = $arResult["PAY_SYSTEM"][$arOrder["PAY_SYSTEM_ID"]]["NAME"];
              }
              
              $objProp = CSaleOrderPropsValue::GetOrderProps($arOrder["ID"]);
              while($arProp = $objProp->Fetch())
              {
                if (in_array($arProp["CODE"], array("PHONE", "ADDRESS", "INDEX", "LOCATION", "CITY")) &&
                    !empty($arProp["VALUE"]))
                {
                    if ($arProp["TYPE"] == "LOCATION")
                    {
                      if (is_array($arProp["VALUE"] = CSaleLocation::GetByID($arProp["VALUE"])))
                      {
                        foreach(array("CITY_NAME", "REGION_NAME", "COUNTRY_NAME") as $v)
                        {
                          if (!empty($arProp["VALUE"][$v]))
                          {
                            $arProp["VALUE"] = $arProp["VALUE"][$v];
                            break;
                          }
                        }
                        
                        if (is_array($arProp["VALUE"]) ||
                            is_numeric($arProp["VALUE"]))
                        {
                          continue;
                        }
                      }
                    ...
                    $OrderProps[] = $arProp["NAME"].": ".$arProp["VALUE"];
                }
              }
            }
            ...
            "ORDER_PROPS" => implode("<br>", $OrderProps) 
					);
