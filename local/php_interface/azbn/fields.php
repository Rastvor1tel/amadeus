...
/*
class AzbnHtmlRedactorType extends CUserTypeString { 
	
	function GetUserTypeDescription() {
		return array(
			'USER_TYPE_ID' => 'c_string', 
			'CLASS_NAME' => 'AzbnHtmlRedactorType', 
			'DESCRIPTION' => 'Текст в html редакторе', 
			'BASE_TYPE' => 'string',
		); 
	}
...
AddEventHandler('main', 'OnUserTypeBuildList', array('AzbnHtmlRedactorType', 'GetUserTypeDescription'));
*/
...
AddEventHandler('main', 'OnUserTypeBuildList', array('Azbn7CUserTypeIBlockElementList', 'GetUserTypeDescription'), 5000);
class Azbn7CUserTypeIBlockElementList extends CUserTypeIBlockElement {
	...
	function GetUserTypeDescription() {
		return array(
			...
			'USER_TYPE_ID' => 'iblock_element',//'iblock_element_list',
			// имя класса, методы которого формируют поведение типа
			'CLASS_NAME' => 'Azbn7CUserTypeIBlockElementList',
			// название для показа в списке типов пользовательских свойств
			'DESCRIPTION' => 'Связь с элементами инфоблока (Azbn7)',
	...
	function GetEditFormHTML($arUserField, $arHtmlControl) {
		...
		$arElements = Azbn7CUserTypeIBlockElementList::_getElements($arHtmlControl['VALUE']);
		// html поля веб-формы для текущего значения
		$sReturn = $sReturn . '<div>'.Azbn7CUserTypeIBlockElementList::_getItemFieldHTML($arHtmlControl['VALUE'], $iIBlockId, $arElements, $arHtmlControl['NAME']).'</div>';
		return $sReturn;
		...
		if(!empty($mElementId)) {
			...
			if(!empty($arFilter['ID'])) {
				...
				while($arItem = $rsItems->GetNext(false, false)) {
					$arItem['NAME'] = $arItem['ID'] . $arItem['NAME'];
