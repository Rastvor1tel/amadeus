...
// if(isset($arParams["CUSTOM_FILTER"]) && is_array($arParams["CUSTOM_FILTER"])){
// 	$arrFilter = array_merge($arrFilter, $arParams["CUSTOM_FILTER"]);
// }
...
if($this->StartResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation, $pagerParameters)))
{
    ...
    if ((!isset($arFilter["SECTION_ID"]) || 
         intval($arFilter["SECTION_ID"]) <= 0) &&
        isset($arrFilter["FACET_OPTIONS"]))
    {
      unset($arrFilter["FACET_OPTIONS"]);
    }
	...
	// if(isset($arParams["CUSTOM_FILTER"]) && is_array($arParams["CUSTOM_FILTER"])){
// 		var_dump(array_merge($arrFilter, $arFilter));
// 		die();
// 	}
