...
<?if($arResult['CAN_EDIT_PASSWORD']):?>
	<tr>
		<td><?=GetMessage('NEW_PASSWORD_REQ')?><p style="font-weight:normal;font-size: 14px;"><?echo $arResult["GROUP_POLICY"]["PASSWORD_REQUIREMENTS"];?></p></td>
...
</div><div class="cabinet__delivery edit-profile">
        <p><b>Адреса доставки:</b></p>
    <? $count = 0;
    foreach ($arResult["arUser"]['UF_DELIVERYADRESS'] as $key => $arAdress) {
        $count++;?>
        <div class="cabinet__row">
            <div class="cabinet__cell">Адрес <?=$count?></div>
            <input type="text" name="UF_DELIVERYADRESS[]" value="<?=$arAdress?>" size="50" class="form__text fields string cabinet__cell">
        </div>
        <?/*<input type="text" name="UF_DELIVERYADRESS[]" value="<?=$arAdress?>" size="50" class="form__text fields string">*/?>
    <?}?>
    <div class="cabinet__row">
        <div class="cabinet__cell">Адрес <?=$count+1?></div>
        <input type="text" name="UF_DELIVERYADRESS[]" value="" size="50" class="form__text fields string cabinet__cell">
    </div>
</div>
